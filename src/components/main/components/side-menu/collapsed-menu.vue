<template>
  <Dropdown ref="dropdown" @on-click="handleClick" :class="{'collased-menu-dropdown': !hideTitle}" :transfer="hideTitle" :placement="placement">
    ...
    <DropdownMenu ref="dropdown" slot="list">
      <template v-for="child in children">
        ...
        <DropdownItem v-else :key="`drop-${child.name}`" :name="child.name" :selected="activeName === child.name"><common-icon :size="iconSize" :type="child.icon"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>
