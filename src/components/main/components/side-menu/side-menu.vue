    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        <collapsed-menu v-if="item.children && item.children.length > 1"
                        class="menus"
                        :class="{'sbg': include(item.children)}"
                        @on-click="handleSelect"
                        hide-title
                        :active-name="activeName" :root-icon-size="rootIconSize" :icon-size="iconSize" :theme="theme"
                        :parent-item="item" :key="`drop-menu-${item.name}`"></collapsed-menu>
        <Tooltip transfer v-else
                 class="menus"
                 :class="{'sbg': activeName === item.name + '_page'}"
